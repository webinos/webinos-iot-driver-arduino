...
//                                    xbee.print("\"}}");
        ...
        //xbee.println("error opening configuration file");
    }
}


void connect_to_pzp(){
    xbee.print("<{\"cmd\":\"con\", \"id\":\"");
    xbee.print(board_id);
    xbee.println("\",\"protocol\":\"xbee\",\"name\":\"ARDUINO_MEGA\"}>");
}
...
void setup(){
    ...
    /*
    * init zigbee
    * note: adapter must be attached as follow: tx->13, rx->12, 5v->11
    */
    pinMode(11,OUTPUT);
    ...
    while(board_id == NULL){
         xbee.println("Please disconnect and reconnect the board");
         err_SD();
         send_ack("err","-1","sd");
         getInfoFromSD(false);
    }
...
void loop(){ 
    ...
    if(xbee.available()>0){
        ...
        if(strcmp(cmd, CONNECTED) == 0){
            //xbee.print(ELEMENTS_RES);
            xbee.print("<{\"id\":\"");
            xbee.print(board_id);
            xbee.print("\",\"cmd\":\"ele\",\"elements\":");
            getInfoFromSD(true);
            xbee.println("}>");
        }
