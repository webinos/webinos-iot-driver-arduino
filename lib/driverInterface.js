...
(function () {
    ...
    var driverInterface = function() {
        ...
        function loadDrivers(){
            ...
            var settings = JSON.parse(fs.readFileSync(filePath).toString());
            var drivers = settings.params.drivers;
            for(var i in drivers){
                //if(drivers[i].type != "http"){
                    try {
                        //console.log("Try to load " + (driversLocation + drivers[i].filename));
                        var newDriver = require(driversLocation + drivers[i].filename);
                        driversList[newDriverId++] = newDriver;
                    }
                    catch(e) {
                        console.log('Error: cannot load driver '+drivers[i].filename);
        ...
        function loadDrivers() {
            ...
            for(var i in fileList) {
                ...
                console.log("load " +  driversLocation+fileList[i]);
                try {
...
//            console.log('connect sa is '+sensorActuator);
            ...
            //if (allListenersLoaded) { //this should be removed when adding actuators
                initDrivers();
            //}
        }
