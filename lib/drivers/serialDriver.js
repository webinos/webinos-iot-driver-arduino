 ...
 (function () {
    ...
    //                      board["language"] = data.language;
                            ...
                            for(var i=0; i<data.elements.length ;i++){   
                                ...
                                if(!isAlreadyRegistered(tmp_ele.id)){
                                    ...
                                    tmp_ele.element.description = tmp_ele.element.type + " "  + str_type + " (on " + boards[data.id].name + ")" + ", id: " + tmp_ele.id;
                                    try{
                            ...
                            var fake_sensor = {};
                            fake_sensor["type"] = "test";
                            fake_sensor["name"] = "fake_arduino_test_sensor_serial";
                            fake_sensor["sa"] = 0;
                            fake_sensor["description"] = "Fake arduino sensor for test purpose";
                            var id = registerFunc(driverId, fake_sensor["sa"], fake_sensor);
                            elementsList[id] = {element:fake_sensor};

                            var fake_actuator = {};
                            fake_actuator["type"] = "test";
                            fake_actuator["name"] = "fake_arduino_test_actuator_serial";
                            fake_actuator["sa"] = 1;
                            fake_actuator["description"] = "Fake arduino actuator for test purpose";
                            var id = registerFunc(driverId, fake_actuator["sa"], fake_actuator);
                            elementsList[id] = {element:fake_actuator};
    ...
    */
    ...
    exports.execute = function(cmd, eId, data, errorCB, successCB) {
        var wait_time = 50 * elementsList.length;
        ...
        switch(cmd) {
            case 'cfg':
                ...
                setTimeout(send_command, (k++)*wait_time, output_port, CONFIGURE_CMD,native_element_id,param_data);
                break;
            case 'start':                                 
                ...
                setTimeout(send_command, (s++)*wait_time, output_port, START_LISTENING_CMD,native_element_id,NO_VALUE);
                break;
            case 'stop':
                ...
                setTimeout(send_command, (w++)*wait_time, output_port, STOP_LISTENING_CMD,native_element_id,NO_VALUE);
                break;
            case 'value':
                //In this case the actuator should store the value
                //the parameter data is the value to store  
                console.log('Sent value for actuator '+eId+'; value is '+ JSON.stringify(data));
                send_command(output_port, SET_ACTUATOR_VALUE_CMD,native_element_id,data.actualValue[0]);
