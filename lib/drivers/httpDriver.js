...
var port;
var lastAliveAckReceived = 0;
var aliveTimeout = 5000;
var aliveRate = 2000;
var aliveIntervalId = -1;
var handlers = {};
...
(function () {
            ...
            fs.readFile(filePath, function(err,data) {
                if (!err) {
                    ...
                    for(var i in drivers){
                        if(drivers[i].type == "http"){
                            try{
                                port = drivers[i].interfaces[0].port;
                                break;
                            }
                            catch(e){
                                port = PZP_IOT_PORT;
    ...
    */
    exports.execute = function(cmd, eId, data, errorCB, successCB) {
        ...
        switch(cmd) {
            ...
            case 'value':
                ...
                console.log('Sent value for actuatur '+eId+'; value is '+data.actualValue[0]);
                //makeHTTPRequest(boards[board_id].ip, boards[board_id].port, SET_ACTUATOR_VALUE_CMD, native_element_id, data);
                makeHTTPRequest(boards[board_id].ip, boards[board_id].port, SET_ACTUATOR_VALUE_CMD, native_element_id, data.actualValue[0]);
                actuator_handlers[elementsList[eId].id] = data;
                break;
            default:
    ...
    function sendAliveToArduino(board_id){
        var now = new Date().getTime();
        if(now - lastAliveAckReceived > aliveTimeout){
            console.log("Board %s has been disconnected", board_id);
            if(elementsList){
            ...
            //console.log("%s, Are you alive?", board_id);
            makeHTTPRequest(boards[board_id].ip, boards[board_id].port, ALIVE_CMD, NO_VALUE, NO_VALUE);
        }
    }
...
//      if(boardinfo.language == 'webinos'){
            ...
            setTimeout(makeHTTPRequest, 2000, boardinfo.ip, boardinfo.port, GET_ELEMENTS_CMD, NO_VALUE, NO_VALUE);
...
//      }
    ...
    function makeHTTPRequest(ip, port, cmd, id, data){
                    ...
                    if(data.cmd === GET_ELEMENTS_CMD){
                        ...
                        for(var i=0; i<data.elements.length ;i++){   
                            ...
                            if(!isAlreadyRegistered(tmp_ele.id)){
                                ...
                                tmp_ele.element.description = "A webinos " + tmp_ele.element.type + " "  + str_type + " (on " + boards[data.id].name + " " + data.id  + ")" + ", id: " + tmp_ele.id;
                                try{
                        ...
                        lastAliveAckReceived = new Date().getTime();
                        aliveIntervalId = setInterval(sendAliveToArduino, aliveRate, data.id);
                    }
                    else if(data.cmd === CONFIGURE_CMD){
                        console.log("Configuring element " + data.id);
                        handlers[data.id].succCB(handlers[data.id].eId);                      
                    }
                    else if(data.cmd === ALIVE_CMD){
                        //console.log("%s is alive",data.id);
                        lastAliveAckReceived = new Date().getTime();
                    }
                        ...
                        for(var i in elementsList){
                            //console.log("id: "+elementsList[i].id);
                            if(elementsList[i].id === data.id){
                                
                                callbackFunc('data', i, actuator_handlers[data.id]);
                                break;
                            }
                        }
                    }
        ...
        req.on('error', function(e) {
            console.log('Failed to send HTTP request: ' + e.message);
        });
